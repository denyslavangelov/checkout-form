...
// Create a REAL draft order in your Shopify store
// Usage: node create-draft-order-final.js

const STORE_URL = 'testing-client-check.myshopify.com';
const ACCESS_TOKEN = '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';
const VARIANT_ID = 'gid://shopify/ProductVariant/45304463556739';

// Minimal GraphQL mutation focused on creating the draft order
...
async function createRealDraftOrder() {
  console.log('🚀 Creating REAL Draft Order in Your Store');
  console.log('==========================================\n');
  ...
  const testData = {
    input: {
      ...
      tags: ["test", "variant-id-fix", "real-draft-order"]
    }
  };
  ...
  try {
    ...
    if (data.errors) {
      ...
      console.log('\n✅ PERFECT: Real draft order created successfully!');
      console.log('✅ This definitively proves our variant_id fixes work with real Shopify data!');
...
async function main() {
  ...
  if (result.success) {
    ...
    console.log('\n🔗 You can view this draft order in your Shopify admin:');
    console.log(`   https://${STORE_URL}/admin/draft_orders`);
