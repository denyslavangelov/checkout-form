...
interface StandaloneOfficeSelectorProps {
  productId: string;
  variantId: string;
  onClose: () => void;
...
export function StandaloneOfficeSelector({ 
  productId, 
  variantId, 
  onClose, 
  onOrderCreated 
}: StandaloneOfficeSelectorProps) {
    ...
    return new Promise((resolve) => {
      ...
      if (window.parent) {
        ...
        const messageHandler = (event: MessageEvent) => {
          console.log('🏢 Received message in standalone component:', event.data);
  ...
  const searchCities = async (term: string) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/speedy/search-site`, {
        method: 'POST',
        headers: {
      ...
      const cities = data.sites?.map((site: any) => ({
        id: site.id,
        name: site.name,
      ...
      setError('Грешка при търсене на градове');
    } finally {
  ...
  const searchOffices = async (cityId: string, term: string) => {
    if (!cityId || term.length < 2) {
      setOffices([]);
      return;
    }
    ...
    try {
      ...
      setError('Грешка при търсене на офиси');
    } finally {
  ...
  const debouncedSearchCities = React.useCallback(
    debounce((term: string) => searchCities(term), 300),
    []
  );

  const debouncedSearchOffices = React.useCallback(
    debounce((cityId: string, term: string) => searchOffices(cityId, term), 300),
    []
  );
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      if (productId === 'cart' && variantId === 'cart') {
        ...
        // Get fresh cart data from the parent window right before creating the order
      ...
      setError(err instanceof Error ? err.message : 'Грешка при създаване на поръчката');
    } finally {
    ...
    <div className="bg-white rounded-lg p-4 sm:p-6 max-w-md w-full mx-2 sm:mx-4 relative shadow-lg border border-gray-200">
      ...
      <div className="space-y-4">
            ...
            <div className="relative office-dropdown">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 pl-8 pr-8 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent disabled:bg-gray-100"
                />
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        ...
        <button
          ...
          className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white py-3 sm:py-4 text-sm sm:text-base font-medium rounded-md transition-colors"
        >
          {creatingOrder ? (
            <>
