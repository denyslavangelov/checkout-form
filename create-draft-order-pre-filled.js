...
// Create a draft order with BOTH address and country pre-filled
// Usage: node create-draft-order-pre-filled.js
...
// Random addresses for Bulgaria (your only market)
const randomAddresses = [
  {
    address1: "ул. Витоша 1",
    country: "Bulgaria"
  },
  {
    address1: "бул. Цариградско шосе 125", 
    country: "Bulgaria"
  },
  {
    address1: "ул. Граф Игнатиев 15",
    country: "Bulgaria"
  },
  {
    address1: "бул. България 45",
    country: "Bulgaria"
  },
  {
    address1: "ул. Раковска 78",
    country: "Bulgaria"
  }
];

// GraphQL mutation to create draft order with pre-filled address and country
...
async function createDraftOrderWithPreFilledFields() {
  console.log('📦 Creating Draft Order with PRE-FILLED Address and Country');
  console.log('=========================================================\n');
  ...
  console.log('🏠 PRE-FILLING with Random Address:');
  console.log(`   Address: ${randomAddress.address1}`);
  console.log(`   Country: ${randomAddress.country}`);

  try {
    console.log('\n🚀 Creating draft order with PRE-FILLED address and country...');
      ...
      body: JSON.stringify({
        ...
        variables: {
          input: {
            ...
            tags: ["pre-filled", "test", "bulgaria-market", "address-country-filled"]
          }
        }
      })
    });
    ...
    if (data.data?.draftOrderCreate?.draftOrder) {
      ...
      console.log('\n🎉 SUCCESS: DRAFT ORDER CREATED WITH PRE-FILLED FIELDS!');
      console.log('====================================================');
      ...
      console.log('\n🏠 Shipping Address (PRE-FILLED FIELDS):');
      if (draftOrder.shippingAddress) {
        const addr = draftOrder.shippingAddress;
        console.log(`   Address: ${addr.address1 || '❌ NOT FILLED'}`);
        console.log(`   Country: ${addr.country || '❌ NOT FILLED'}`);
        ...
        // Check if both fields are actually filled
        if (addr.address1 && addr.country) {
          console.log('\n✅ PERFECT: BOTH ADDRESS AND COUNTRY ARE PRE-FILLED!');
          console.log('✅ Address field is filled:', addr.address1);
          console.log('✅ Country field is filled:', addr.country);
        } else {
          console.log('\n❌ ISSUE: Fields are not properly pre-filled!');
          console.log('❌ Address filled:', !!addr.address1);
          console.log('❌ Country filled:', !!addr.country);
        }
      } else {
...
async function main() {
  await createDraftOrderWithPreFilledFields();
  
  console.log('\n🏁 Draft order creation completed!');
  console.log('\n📝 This script ensures:');
  console.log('   ✅ Address field is PRE-FILLED in the draft order');
  console.log('   ✅ Country field is PRE-FILLED in the draft order');
  console.log('   ✅ Both fields will appear filled when customer views checkout');
  console.log('   ✅ Customer can still modify if needed');
}
